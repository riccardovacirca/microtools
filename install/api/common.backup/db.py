...
def helper_get_pool_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "max_overflow": int(get_env("DB_POOL_MAX_OVERFLOW", "10")),
        "pool_timeout": int(get_env("DB_POOL_TIMEOUT", "30")),
        "pool_recycle": int(get_env("DB_POOL_RECYCLE", "3600")),
        "pool_pre_ping": get_env("DB_POOL_PRE_PING", "true").lower() == "true",
        "echo_pool": get_env("DB_POOL_ECHO", "false").lower() == "true"
    }


def helper_create_engine(driver: str) -> Engine:
    ...
    """
    pool_config = helper_get_pool_config()

    if driver == "sqlite3":
        db_path = get_env("SQLITE3_CONNECTION", "").replace("file:", "")
        if not db_path:
        ...
        conn_str = get_env("PGSQL_CONNECTION")
        if not conn_str:
        ...
        conn_str = get_env("MARIADB_CONNECTION")
        if not conn_str:
...
def helper_get_engine(driver: str | None = None) -> Engine:
    ...
    if driver not in _engines:
        _engines[driver] = helper_create_engine(driver)
...
def helper_dispose_all_pools():
    ...
    Example:
        ...
        async def shutdown_event():
            from api.common.db import helper_dispose_all_pools
            helper_dispose_all_pools()
    """
...
def helper_get_pool_status(driver: str | None = None) -> Dict[str, Any]:
    ...
    """
    try:
        engine = helper_get_engine(driver)
        pool = engine.pool
...
def db_cursor(driver: str | None = None, autocommit: bool = True):
    ...
    Example:
        ...
        # Explicit driver
        with db_cursor(driver="pgsql") as cursor:
            cursor.execute("SELECT version()")
    """
    with db_connection(driver) as conn:
        cursor = conn.cursor()
        try:
            yield cursor
            if autocommit:
...
# ==============================================================================
# Legacy API Compatibility (get_db_connection)
# ==============================================================================
...
def get_db_connection(driver: str | None = None) -> tuple[Any, str]:
    ...
    Note:
        Prefer using db_connection(), db_cursor(), or db_transaction()
